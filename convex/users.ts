...
export const deleteProfile = mutation({
  handler: async (ctx) => {
    ...
    // 9. Delete all letters involving the user (sent and received)
    ...
    // 10. Delete all notifications involving the user
    ...
    // 11. Delete all reports made by the user or about the user
    ...
    // 12. Delete user data from all tables
    if (user.profilePicture) {
    ...
    // 13. Delete from auth tables (in correct order to avoid foreign key issues)
...
export const blockUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 9. Delete all letters between the users (both directions)
