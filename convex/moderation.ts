 ...
 * MIGRATION NOTES:
 *
 * This file implements a unified reports system that replaces separate
 * reportedUsers and reportedPosts tables with a single reports table.
 *
 * If migration from old tables is needed, use this pattern:
 *
 * // For existing reportedUsers records:
 * await ctx.db.insert("reports", {
 *   reporterId: oldRecord.reporterId,
 *   reportType: oldRecord.reportType,
 *   reportReason: oldRecord.reportReason,
 *   attachment: oldRecord.attachment,
 *   targetType: "user",
 *   targetUserId: oldRecord.reportedUserId,
 *   // targetPostId remains undefined
 * });
 *
 * // For existing reportedPosts records:
 * await ctx.db.insert("reports", {
 *   reporterId: oldRecord.reporterId,
 *   reportType: oldRecord.reportType,
 *   reportReason: oldRecord.reportReason,
 *   attachment: oldRecord.attachment,
 *   targetType: "post",
 *   targetUserId: oldRecord.reportedUserId, // post owner
 *   targetPostId: oldRecord.postId,
 * });
 */
...
export const reportUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingReport = await ctx.db
      .query("reports")
      .withIndex("by_reporter_targetUser", (q) =>
        q.eq("reporterId", reporterId).eq("targetUserId", args.reportedUserId),
      )
      .first();
...
export const reportPost = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existingReport = await ctx.db
      .query("reports")
      .withIndex("by_reporter_targetPost", (q) =>
        q.eq("reporterId", reporterId).eq("targetPostId", args.postId),
      )
      .first();
...
// Get all reports with unified interface
export const getAllReportsWithDetails = query({
  args: {
    limit: v.optional(v.number()),
    targetType: v.optional(v.union(v.literal("user"), v.literal("post"))),
  },
  handler: async (ctx, args) => {
    ...
    // Query reports with optional type filter
    const reportsQuery = args.targetType
      ? ctx.db
          .query("reports")
          .withIndex("by_targetType", (q) =>
            q.eq("targetType", args.targetType as "user" | "post"),
          )
      : ctx.db.query("reports");

    const reports = await reportsQuery.order("desc").take(limit);

    // Enrich reports with details
    const enrichedReports = await Promise.all(
      reports.map(async (report) => {
        ...
        if (report.targetType === "user" && report.targetUserId) {
          ...
          const reportedUser = await ctx.db.get(report.targetUserId);
        ...
        } else if (report.targetType === "post" && report.targetPostId) {
          // Get post details
          const post = await ctx.db.get(report.targetPostId);
          const postOwnerUser = report.targetUserId
            ? await ctx.db.get(report.targetUserId)
            : null;
          ...
          targetDetails = {
            ...
            postOwner: {
              ...
              profilePicture: postOwnerUser?.profilePicture
                ? await r2.getUrl(postOwnerUser.profilePicture)
                : null,
            },
          };
        }
...
// Resolve any report (unified function)
export const resolveReport = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    return { success: true, targetType: report.targetType };
  },
});
...
export const deleteUserAndResolveReport = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Validate this is a user report
    if (report.targetType !== "user" || !report.targetUserId) {
      throw new Error("This is not a valid user report");
    }
    ...
    // 8. Delete all letters involving the user (sent and received)
...
export const deletePostAndResolveReport = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Validate this is a post report
    if (report.targetType !== "post" || !report.targetPostId) {
      throw new Error("This is not a valid post report");
    }
    ...
    if (post) {
      ...
      const postComments = await ctx.db
        .query("comments")
        .withIndex("by_postId", (q) => q.eq("postId", report.targetPostId!))
        .collect();
